...
count=0
MAX_ATTEMPTS=6
ELASTICSEARCH_USERNAME=$(echo $ELASTICSEARCH_USERNAME)
ELASTICSEARCH_PASSWORD=$(echo $ELASTICSEARCH_PASSWORD)
KIBANA_URL=$(echo $KIBANA_URL)
DeploymentID=$(echo $DeploymentID)
DeploymentVersion=$(echo $DeploymentVersion)

echo $ELASTICSEARCH_USERNAME
echo $ELASTICSEARCH_PASSWORD
echo $KIBANA_URL
echo $DeploymentID
echo $DeploymentVersion
...
while [[ "$count" -lt "$MAX_ATTEMPTS" ]]; do
  ...
  http_code=$(echo $estable | grep -oP '}}\K\d+')
  echo $http_code
  ...
  if [[ "$http_code" -eq 200 ]]; then
    ...
    response=$(curl -XGET -H "kbn-xsrf: true" -H 'elastic-api-version: 1' -u ${ELASTICSEARCH_USERNAME}:${ELASTICSEARCH_PASSWORD} -s "${KIBANA_URL}/api/fleet/enrollment-api-keys" -H 'Content-Type: application/json')
    echo $response | jq
